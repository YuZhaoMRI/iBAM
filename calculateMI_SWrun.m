...
run_label='run-3'; % run-1: logical; run-2: Semantic​​; run-3: Visual
if run_label=='run-1' 
   Events_file='logical.txt';      nii_name='Language_filtered_func_data_run-1_clean_noise_second_tr.nii.gz';  
end
if run_label=='run-3'
   Events_file='geometrical.txt';          nii_name='Language_filtered_func_data_run-3_clean_noise_second_tr.nii.gz';  
end
...
% load fMRI volumes 
nii_file=fullfile(sublist(m).folder, sublist(m).name, run_label, nii_name);
dataNii=load_nii(nii_file);
TR=dataNii.original.hdr.dime.pixdim(5);
NumRestFrame=ceil(durRest/TR);
fMRIData4D=double(dataNii.img(:,:,:,1+dumypoints:end-NumRestFrame));% remove the dummy and resting-state fMRI scans at the end 
...
events_file=fullfile(sublist(m).folder, sublist(m).name, run_label, 'Events',Events_file);
...
fMRIData2D(size(TaskTimings,2)+1:end,:)=[];
...
for n=1:numWindFrame  
display(['Frame #',num2str(n)])
...
MI_save=zeros(prod(brainMaskDim3D),numWindFrame);%
MI_save(brainIndex,:)=ModeIntensity2D;
MI_save=reshape(MI_save,[brainMaskDim3D,numWindFrame]);

%% label the corner of images with task timings;  
TaskTimings(numWindFrame+1:end)=[];
...
templateNii.hdr.dime.dim(5)=numWindFrame;
templateNii.original.hdr.dime.dim(5)=numWindFrame;

f_name=[outputpath_sub,'\',run_label,'_sub',sub,'_SW_MI_visual'];
% save_nii(template_nii,[outputpath '\sub' f_name,'_original_98nei_sub_001_run_1.nii.gz'])
save_nii(templateNii,[f_name, '.nii']);

% templateNii=load_nii('BN_Atlas_246_2mm.nii');
% % template_nii.img(:,:,:,1+dumypoints:wind_frame+dumypoints)=single(MI_save);
% mask=templateNii.img;
% mask(mask>0)=1;
% templateNii2=load_nii('MNI152_T1_2mm_brain.nii');
% % template_nii.img(:,:,:,1+dumypoints:wind_frame+dumypoints)=single(MI_save);
% mask2=templateNii2.img;
% mask2(mask2>0)=1;
% mask2=uint8(mask2);
% templateNii.img=mask.*mask2;
% 
% save_nii(templateNii, 'GrayMatterMask.nii')
