...
%   Note: This directory must also contain the corresponding task timings files.
%                                         
% OUTPUTS:
...
fileName_Events='linguistic.txt';
fileName_TaskfMRI='task_func_data_linguistic_SNfiltered.nii.gz';
...
%% load structure noise filtered task fMRI data
% for test dataset
...
fileName_events=fullfile(sublist(isub).folder, sublist(isub).name,  'Events',fileName_Events);
Descent_dur=10;% second,Descent duration of HRF
TaskTimings = generateTaskTimingVector(fileName_events, TR, Descent_dur, NumDummyPoints);


%% correct baselines 
[fMRIData2D, baseLineInfo] = correctBaselineiBAM(fMRIData2D);
...
numTotalWindows  = size(fMRIData2D, 1) -length(slidingWindow) + 1;
scaleFactor=10;
ModeIntensity2D=[];
...
%  1 indicates  task-on periods;and 0 indicates rest.
...
f_name=[outputpath_sub,'\','sub',sub,'_SW_MI'];
