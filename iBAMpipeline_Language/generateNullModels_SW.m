...
%       Note: This directory must also contain the corresponding Movement Regressors.                       
%   OUTPUTS:
...
inputpath = 'H:\Language_fMRI_data';
...
%% Set the following variables according to the structure and file naming settings of your fMRI data and task events storage paths.
fileName_TaskfMRI='func_data_rest__NSSNfiltered.nii.gz';  


%% outputpath
outputpath='F:\Language_task_iBAM_results';
...
display(['generate null models for subject #',num2str(isub)])
...
niiPath=fullfile(sublist(isub).folder, sublist(isub).name, fileName_TaskfMRI);
dataNii=load_nii(niiPath);
...
subDir = fullfile(sublist(isub).folder, sublist(isub).name);
hcpFiles = dir(fullfile(subDir, '*Movement_Regressors.txt'));
fmriprepFiles = dir(fullfile(subDir, '*confounds_timeseries.tsv'));

if ~isempty(hcpFiles)
    if numel(hcpFiles) > 1
        error('More than one Movement_Regressors.txt file found in: %s', subDir);
    end
    movementRegressorFile = fullfile(hcpFiles(1).folder, hcpFiles(1).name);
    motionLabels = censorMotionHCP(movementRegressorFile);
elseif ~isempty(fmriprepFiles)
    if numel(fmriprepFiles) > 1
        error('More than one confounds_timeseries.tsv file found in: %s', subDir);
    end
    confoundsFile = fullfile(fmriprepFiles(1).folder, fmriprepFiles(1).name);
    motionLabels = censorMotionfMRIPrep(confoundsFile);
else
    motionLabels = true(dataNii.original.hdr.dime.dim(5), 1);
    warning('No motion censoring was performed because no motion regressor/confounds file was found.');

end

retainedFramesIdx = find(motionLabels);
% remove the dummy scans
retainedFramesIdx(retainedFramesIdx < 1 + NumDummyPoints) = [];
...
fMRIData2D=zscoreTimeSeries(fMRIData2D);


%% remove the baselines of fMRI data
infoFile=[outputpath_sub,'\','sub',sub,'_SW_baseLineInfo.mat'];
load(infoFile);
[fMRIData2D] = correctBaselineNullModels(fMRIData2D,baseLineInfo);
fMRIData2D(isnan(fMRIData2D))=0;
fMRIData2D(isinf(fMRIData2D))=0;
...
dataInSlidingWindow=fMRIData2D(index_SlidingWindow,:);
...
f_name=[outputpath_sub,'\','sub',sub,'_SW_MI_NullModels'];
