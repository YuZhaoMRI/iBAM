function [Yc] = correctBaselineNullModels(Y, baseLineInfo,opts)
%correctBaselineNullModels Performs robust baseline correction and noise normalization on fluorescence traces.
%
%   [Yc] = correctBaselineNullModels(Y, baseLineInfo,opts) applies a robust
...
%                            .baselines - Estimated baseline   [1 × N].
%                            .sigma     - Estimated noise std  [1 × N].
...
%
    ...
    %% 4) Normalize by estimated noise from task fmri data(unit variance)
    sigma_sw=baseLineInfo.sigma;
    Yc = Yc ./ repmat(sigma_sw, T, 1);
