...
durRest = 10 * 60;        % Duration of the resting-state fMRI scan (seconds)
...
%% Set flags to determine which experimental runs will be processed and analyzed
nii_name='Language_filtered_func_data_run-3_clean_neuro_and_noise_second.nii.gz';  
...
fMRIData4D=double(dataNii.img(:,:,:,1+NumRestFrame:end));% remove the dummy and resting-state fMRI scans at the end 
...
load(baseLineInfoFile);
[fMRIData2D] = nullModelBaselineCorrect(fMRIData2D,baseLineInfo);
...
f_name=[outputpath_sub,'\',run_label,'_sub',sub,'_SW_MI_NullModels'];
