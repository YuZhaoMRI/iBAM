...
function iBAMstatisticalInference()
% iBAMSTATISTICALINFERENCE Non-parametric statistical inference engine for iBAM task activations.
%
%   SYNTAX:
%       iBAMstatisticalInference()
%
%   DESCRIPTION:
...
%
%   INTERNAL CONFIGURATION (Specified within the function body):
...
niiFileName=[outputpath_sub,'\','Emotion_sub',sub,'_SW_MI_NullModels.nii'];
...
%% load task MI maps
niiFileName=[outputpath_sub,'\','Emotion_sub',sub,'_SW_MI.nii'];
niiData=load_nii(niiFileName);
...
parfor i = 1:length(brainIndex)
    ...
    % non-parametric Median Absolute Deviation (MAD) approach with a tuned ThresholdFactor of 1.5.
    nullOutlierMask = isoutlier(currentNullDistribution,'median', 'ThresholdFactor', 1.5);
...
leftArea(leftArea < 0.0000001) = 0.0000001;
leftArea(leftArea > 0.9999999) = 0.9999999;
...
niiZmapFileName_out = sprintf('%s\\Emotion_sub%s_SW_Zmap_thresholded_FDR%.2f.nii', ...
...
niiZmapFileName_out=[outputpath_sub,'\','Emotion_sub',sub,'_SW_Zmap_raw.nii'];
save_nii(niiData,niiZmapFileName_out);
